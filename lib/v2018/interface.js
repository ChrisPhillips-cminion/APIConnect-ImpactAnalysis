...
function getSubscriptions(prod, planObj, catalog, config, cb ) {
  getSubscriptions(prod, planObj, catalog, config, cb,1)
}

function getSubscriptions(prod, planObj, catalog, config, cb,no) {
      ...
      body.results.forEachThen(function(cb, consumerorg) {
        ...
        request.get(options, function(err, response, body) {
          ...
          if (!err) {
            ...
            {
              if (config.debug) {
                console.debug("URL ",url," returned Error",no,err)
              }
              getSubscriptions(prod, planObj, catalog, config, cb,no++);
            } else {
              console.log("Error ",no,"Max error count reached, url",url," ABENDING",err)
              process.exit(99)
            }
