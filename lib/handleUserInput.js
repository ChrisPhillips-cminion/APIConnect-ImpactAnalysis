...
function handleUserInput(config, callback) {
    ...
    function getOrgs(config, callback) {
        ...
        orgSpinner.start();
        f.call(apicpath+' organizations --server ' + config.server, function (err, stdout, stderr) {
            if (err) {
                console.error(err);
                orgSpinner.fail();
                throw err;
    ...
    function getCats(config, callback) {
        ...
        catSpinner.start();
        f.call(apicpath+' catalogs --server ' + config.server + ' --organization ' + config.organization, function (err, stdout, stderr) {
            if (err) {
                catSpinner.fail();
                console.error(err);
                throw err;
            ...
            inquirer.prompt(questionCat).then(function (cats) {
                ...
                cats.catalog.forEach(function (cat) {
                    calls.push(function (callbackCat) {
                        ...
                        catName.start();
                        f.call(apicpath+' catalogs:get ' + cat + ' --server ' + config.server + ' --organization ' + config.organization, function (err, stdout, stderr) {
                            if (err) {
                                catName.fail();
                                console.error(err);
                                throw err;
