...
function handleUserInput(config, callback) {
    ...
    function getDisplaySchema(config, callback) {
        ...
        if (config.displaySchema === undefined) {
            ...
            displaySchema.push('Service - Server / QueueManager');
    ...
    function getOrgs(config, callback) {
        ...
        orgSpinner.start();
        f.call('apic organizations --server ' + config.server, function (err, stdout, stderr) {
            if (err) {
                console.error(err);
                orgSpinner.fail();
                throw err;
    ...
    function getCats(config, callback) {
        ...
        catSpinner.start();
        f.call('apic catalogs --server ' + config.server + ' --organization ' + config.organization, function (err, stdout, stderr) {
            if (err) {
                catSpinner.fail();
                console.error(err);
                throw err;
