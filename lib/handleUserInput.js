...
function handleUserInput(e, cb) {
  ...
  function t(config, r) {
    config.version = "v2018"
    // var spin = f.makeSpinner("Logging in...");
    if (config.debug) { console.error("Begin apicLogin function") } 
    require("./" + config.version + "/interface.js").apicLogin(config.username, f.b64decode(config.password), config.server, config, function(e) {
