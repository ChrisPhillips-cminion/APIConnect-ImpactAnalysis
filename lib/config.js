...
var fs = require("fs"),
    path = require("path"),
    validFlags = ["--help", "-h", "--load", "--json","--csv", "--debug"],
    homeDir = process.cwd(),
    config = {
        detailed: !1,
        tempfolder: "tmp",
...
if ((isFlag("--help") || isFlag("-h")) && (config.showHelp = !0), (isFlag("--debug") || isFlag("-d")) && (config.debug = !0),isFlag("--csv") && (config.output = "csv"), isFlag("--json") && (config.output = "json"), isFlag("--save") || isFlag("-s")) {
