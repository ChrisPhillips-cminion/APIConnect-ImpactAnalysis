...
function process(callback, name, ntype, uri, utype, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app) {
    ...
    if (uri) {
        ...
        service1 = 'OAuth Host - '.bold + urip.protocol + "//" + urip.host;
        service2 = 'Oauth '.bold+utype.bold+' - '.bold + urip.pathname;
    }
    ...
    fet.ObjectForEachThen(displaySchema, function(cb, entry, val, idx) {
            if (val === 'Policy / Service Details 1') {
                ...
                schema['a' + idx] = 'Did not recognise : '.bold + val;
            }
            cb();
        },
        function() {
                    ...
                    plan.apis.forEachThen(function(callback, api) {
                        if (api.policies != []) {
                                ...
                                if (plan.apps.length === 0) {
                                    process(function() {
                                        process(function() {
                                            process(function() {
                                                callback();
                                            }, k, v.type, v.tokenIntrospect, "Introspection URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName);
                                        }, k, v.type, v.tokenUrl, "Token URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName);
                                    }, k, v.type, v.authorizationUrl, "Authorization URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName);
                                } else {
                                    plan.apps.forEachThen(function(callback, app) {
                                        process(function() {
                                            process(function() {
                                                process(function() {
                                                    callback();
                                                }, k, v.type, v.tokenIntrospect, "Introspection URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app.name);
                                            }, k, v.type, v.tokenUrl, "Token URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app.name);
                                        }, k, v.type, v.authorizationUrl, "Authorization URL", api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app.name);
                                    }, callback);
                                }
                            }, callback);
                        } else {
                            if (plan.apps.length === 0) {
                                process(function() {
                                    callback();
                                }, undefined, undefined, undefined, undefined, api, plan, product, displaySchema, ServiceObj, catalogDisplayName);
                            } else {
                                plan.apps.forEachThen(function(callback, app) {
                                    process(function(sobj) {
                                        ServiceObj = sobj;
                                        callback();
                                    }, undefined, undefined, undefined, undefined, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app.name);
                                }, callback);
                            }
                            callback();
                        }
