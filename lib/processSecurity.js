...
function process(callback,output, name, ntype, uri, utype, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app) {
    ...
    if (uri) {
        ...
        service1 = 'OAuth Host - '.boldsw(output) + urip.protocol + "//" + urip.host;
        service2 = 'Oauth '.boldsw(output)+utype.boldsw(output)+' - '.boldsw(output) + urip.pathname;
    }
    else if (ntype != 'oauth2'){
      service1 = ntype.boldsw(output);
      service2 = name;
    }
