...
function process(callback, policy, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app) {
    ...
    if (policy) {
        service1 = "Policy Name - ".bold + policy.name
        service2 = "Policy Version - ".bold + policy.version;
    }
                        ...
                        if (api.policies != []) {
                            api.policies.forEachThen(function(callback, policy) {
                                if (plan.apps.length === 0) {
                                    process(function() {
                                        callback();
                                    }, policy, api, plan, product, displaySchema, ServiceObj, catalogDisplayName);
                                } else {
                                    plan.apps.forEachThen(function(callback, app) {
                                        process(function(sobj) {
                                            ServiceObj = sobj;
                                            callback();
                                        }, policy, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app.name);
                                    }, callback);
                                }
                            }, callback);
                        } else {
