...
function parseYaml(jsonFile, catalog, callback) {

    var error = null,

        // jsonFile = yaml.safeLoad(fs.readFileSync(file, 'utf8')),
        apiName =  jsonFile.info.title,
    ...
    if (jsonFile.product) {
        ...
        let basepath = jsonFile.basePath
        var apiI = new API(apiName, version, apiTitle,basepath);
        results.api = (apiI);
            ...
            try {
                ...
                assembly.forEach(function(e) {
                    ...
                    examineYamlAssembly(e, jsonFile['x-ibm-configuration'], config.catalog, catalog, apiI, function() {
                        ...
                        examineYamlSecurity(jsonFile, assembly, catalog, apiI, function(){});
                    })
                });
