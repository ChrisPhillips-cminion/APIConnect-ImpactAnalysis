...
function process(callback, output, invoke, api, plan, product, displaySchema, ServiceObj, catalogDisplayName, app) {
    ...
    if (invoke) {
        if (invoke.type === 'mq') {
            service1 = 'Service QueueManager - '.boldsw(output) + invoke.qm;
            service2 = 'Service Queue - '.boldsw(output) + invoke.q;
        } else if (invoke.type === 'http') {
            service1 = 'Service Host - '.boldsw(output) + invoke.url.protocol + "//" + invoke.url.host;
            service2 = 'Service ContextRoute - '.boldsw(output) + invoke.url.pathname;
            if (invoke.url.pathname.startsWith('/$(target-url)$(request.path)')) {
                service1 = 'Service Host -'.boldsw(output) + ' $(target-url)';
                service2 = 'Service ContextRoute -'.boldsw(output) + ' $(request.path)';
            }
        }
    }
