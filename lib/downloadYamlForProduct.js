...
function downloadYamlForProduct(config, callbackParent) {
    var root_dir =path.join( process.cwd() , config.tempfolder),
        calls = [];
    ...
    Object.keys(config.catalog).forEach(function (cat) {
        calls.push(function (callbackchild) {
            ...
            f.cd(path.join(root_dir , cat));

            var apicpath = path.join(root_dir,'..','node_modules','.bin','apic')

            console.error('Begin downloadYamlForProduct function for ' + cat);
            var prodList = apicpath+' products -s ' + config.server + ' -c ' + cat + ' --organization ' + config.organization,
                pullYaml = apicpath+' products:pull prodname -s ' + config.server + ' -c ' + cat + '  --organization ' + config.organization;
