...
function downloadYamlForProduct(config, callbackParent) {
    ...
    Object.keys(config.catalog).forEach(function (cat) {
        calls.push(function (callbackchild) {
            ...
            var prodList = root_dir+'/../node_modules/.bin/apic products -s ' + config.server + ' -c ' + cat + ' --organization ' + config.organization,
                pullYaml = root_dir+'/../node_modules/.bin/apic products:pull prodname  -s ' + config.server + ' -c ' + cat + '  --organization ' + config.organization;
