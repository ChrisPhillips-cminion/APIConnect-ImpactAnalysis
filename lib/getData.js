...
function getData(config, callback) {
    function getSubscriptions(tempAppList, cat, callback) {
        call('apic subscriptions -c ' + cat + ' --server ' + config.server + ' --organization ' + config.organization, function(err, stdout, stderr) {
            if (err) {
                console.error(err);
                throw err;
            }
        ...
        Object.keys(config.catalog).forEachThen(
            function(callbackP, cat) {
                ...
                commands.push(function(callbackpp) {
                    ...
                    call('apic catalogs:get ' + cat + ' --server ' + config.server + ' --organization ' + config.organization, function(err, stdout, stderr) {
                        if (err) {
                            console.error(err);
                            throw err;
                        } else if (stderr) {
                            ...
                            fs.readdir('tmp/' + cat, function(err, files) {
                                var calls = [];
                                files.forEachThen(
                                    function(callback, file) {
                                        parseYaml('tmp/' + cat + '/' + file, function(err, results) {
                                            var iurl;
                                            if (results.products.plans === undefined) {
